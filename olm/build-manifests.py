...
def generate_manifests(args: argparse.Namespace) -> None:
    ...
    manifests = generate_helm_templates(product, op_name, args.repo_operator)
    ...
    to_dump.extend(
        filter(
            lambda m: m in [f"{op_name}-configmap", f"{product}-clusterrole"], manifests
        )
    )
    ...
    try:
        ...
        csv_dest.write_text(yaml.dump(csv))

        for y in to_dump:
            manifest_dest = dest_dir / "manifests" / f"{y['metadata']['name']}.yaml"
            manifest_dest.write_text(yaml.dump(y))
...
spec:
    ...
    customresourcedefinitions:
        ...
        - placeholder
    relatedImages:
    - name: operator name placeholder
      image: quay.io image placeholder
    install:
        ...
        spec:
            ...
            clusterPermissions:
            - serviceAccountName: airflow-operator-serviceaccount placeholder
              rules: placeholder
            deployments:
            - name: airflow-operator placeholder
              spec: placeholder
"""
