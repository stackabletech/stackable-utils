...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    args.product = args.repo_operator.name.split("-")[0]
...
def generate_manifests(args: argparse.Namespace) -> list[dict]:
    ...
    op_cluster_role = next(
        filter(
            lambda m: m["metadata"]["name"] == f"{args.op_name}-clusterrole", manifests
        )
    )
    op_service_account = next(
        filter(
            lambda m: m["metadata"]["name"] == f"{args.op_name}-serviceaccount",
            manifests,
        )
    )
    op_deployment = next(
        filter(
            lambda m: m["metadata"]["name"] == f"{args.op_name}-deployment", manifests
        )
    )
