...
@test "assert_tag_not_exists: passes when tag does not exist" {
	...
	run assert_tag_not_exists "origin" "26.3.0"
	[ "$status" -eq 0 ]
	teardown_temp_repo_with_remote_branch
}

@test "assert_tag_not_exists: fails when tag exists on remote" {
	setup_temp_repo_with_remote_branch
	git tag "26.3.0"
	git push origin "26.3.0" --quiet
	run assert_tag_not_exists "origin" "26.3.0"
	[ "$status" -eq 1 ]
...
@test "assert_tag_not_exists: does not match partial tag names" {
	setup_temp_repo_with_remote_branch
	git tag "26.3.0-rc1"
	git push origin "26.3.0-rc1" --quiet
	run assert_tag_not_exists "origin" "26.3.0"
	[ "$status" -eq 0 ]
	teardown_temp_repo_with_remote_branch
}
